    ...
    def precompute_legal_patterns(self):
        if self.use_multiprocessing:
            with Pool(os.cpu_count()) as pool:
                patterns_offsets = [(pattern, offset) for pattern in self.patterns for offset in self.offsets]
                patterns_compatibility = pool.starmap(self.legal_patterns, patterns_offsets)
        else:
            patterns_compatibility = [self.legal_patterns(pattern, offset) for pattern in self.patterns for offset in self.offsets]
